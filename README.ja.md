...
Proto プロトコルファイルを `Server/Unity/TypeScript` コードに変換するツールです。
...
`--mode`
実行モードを指定します。有効な値は `Server`, `Unity`, または `TypeScript` のいずれかです。
...
`--namespaceName`
名前空間を指定します。TypeScript モードではこのパラメータは無効です。名前空間を設定しない場合は空の値を渡してください。
